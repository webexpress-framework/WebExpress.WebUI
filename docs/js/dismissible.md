...
The component is initialised declaratively from `data-` attributes on the host element. The C# side (`ControlDismissible`) sets these attributes automatically; manual HTML usage is possible too.
...
{
    public DetailHint(IFragmentContext fragmentContext)
        : base(fragmentContext)
    {
        Text = _ => "Pick a character above.";
    }
}
```
