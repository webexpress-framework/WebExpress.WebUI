...
- **Interactive Calendar Pop-up**: Clicking the input field or the calendar icon reveals the dropdown menu. The calendar button uses `popovertarget`. The menu enters the browser top layer and CSS anchors determine placement without clipping by overflow containers.
