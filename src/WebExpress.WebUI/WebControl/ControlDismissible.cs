...
namespace WebExpress.WebUI.WebControl
{
    ...
    /// arbitrary controls; in addition, fragment controls registered for the
    /// <see cref="SectionDismissible"/> section are rendered inline.
    /// The section resolves against the runtime type of the panel, so a plugin
    /// that contributes to a panel declares <c>[Scope&lt;ThePanelType&gt;]</c>
    /// and reaches exactly that panel - which is why the control is an
    /// <see cref="IScope"/>.
    /// </summary>
    /// <remarks>
    ...
    /// </remarks>
    public class ControlDismissible : Control, IControlDismissible, IScope
    {
        ...
        public ControlDismissible(string id = null, params IControl[] controls)
            : base(id)
        {
        ...
        public virtual IControlDismissible Add(params IControl[] controls)
        {
            _content.AddRange(controls.Where(x => x is not null));
        ...
        /// <param name="controls">The controls to add.</param>
        /// <returns>The current instance for method chaining.</returns>
        public virtual IControlDismissible Add(IEnumerable<IControl> controls)
        {
            _content.AddRange(controls.Where(x => x is not null));
        ...
        /// <returns>The current instance for method chaining.</returns>
        public virtual IControlDismissible Remove(IControl control)
        {
        ...
        /// followed by the fragment controls registered for the
        /// <see cref="SectionDismissible"/> section - any existing
        /// fragment control (a <c>FragmentControlText</c>, for instance) can be
        /// registered there; no dedicated body fragment type is needed.
        /// </summary>
        ...
        protected virtual IEnumerable<IHtmlNode> RenderBody(IRenderControlContext renderContext, IVisualTreeControl visualTree)
        {
            ...
            // the runtime type is the scope, so a subclass of the panel - not every panel - is
            // what a contributing fragment addresses
            var bodyFragments = fragmentManager?.GetFragments<IFragmentControl, SectionDismissible>(applicationContext, [GetType()]) ?? [];

            var nodes = _content
                .Select(x => x?.Render(renderContext, visualTree))
                .Concat(bodyFragments.Select(x => x.Render(renderContext, visualTree)));
